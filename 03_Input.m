...
% Reactant Flow Rates:
NG_in = 0.000; % kg/hr Natural Gas (assumed 100% CH4)
COAL = 6.264; % kg/hr coal (including moisture and ash)
% see below for more coal-related variables
Primary = 0.691; % fraction of oxidizer to primary combustion zone
% NOTE: The following 4 variables are flow rates through the burner (i.e.
% excluding burnout oxidizer)
Air_in = 32.338*Primary; % kg/hr Air (assumed 1 mole O2 to 3.76 moles N2)
...
P = 101275; % Pressure (Pa) at BYU's elevation
T1 = 555; % initial gas temperature (K)
T2 = 574; % Burnout Oxidizer Pre-heat Temperature (K)
d = 0.250; % diameter of MFR reactor (m)
...
0.435; % The code interpolates when wall temperatures are
1.057; % required between these locations.
1.557; % (linear interpolation)
2.057;
2.557;
3.057;
3.557;
...
1673; % - must correspond to WallX locations.
1573;
1473;
1354;
1300;
...
timax = 3.0; % maximum devolatilization time modeled (seconds)
%----------------------------------------------------------------------
% Proximate and Ultimate Analysis Data for Coal:
% Stored in array yelem in the order: CHNOS, dry, ash-free mass
% fractions
yelem = [0.8193; 0.0469; 0.0093; 0.1148; 0.0097];
ASTMvol = 38.07; % DAF basis (0 < ASTMvol < 100)
...
omegaa = 0.0604; % mass fraction of ash in the parent coal (as received)
